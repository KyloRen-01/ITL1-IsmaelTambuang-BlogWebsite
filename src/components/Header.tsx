...
import {
  Menu, X, LogIn, LogOut, LayoutDashboard, Home, PenSquare,
  Newspaper
} from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ user, currentPage, onNavigate, onAuthClick, onLogout }) => {
          ...
          <nav className="space-y-1">
            <button
              ...
              className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium ${
                currentPage === 'home' ? 'text-blue-400 bg-blue-500/10' : 'text-slate-400'
              }`}
            >
              <Home className="h-4 w-4" />
              Home
            ...
            {user && (
              <>
                <button
                  ...
                  className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium ${
                    currentPage === 'dashboard' ? 'text-blue-400 bg-blue-500/10' : 'text-slate-400'
                  }`}
                >
                  <LayoutDashboard className="h-4 w-4" />
                  Dashboard
                ...
                <button
                  onClick={() => { onNavigate('editor'); setMobileMenuOpen(false); }}
                  className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium ${
                    currentPage === 'editor' ? 'text-blue-400 bg-blue-500/10' : 'text-slate-400'
                  }`}
                >
                  <PenSquare className="h-4 w-4" />
                  New Post
